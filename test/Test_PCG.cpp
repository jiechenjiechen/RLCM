...
int main(int argc, char **argv) {
  ...
  PCG mPCG;
  mPCG.Solve<SMatrix, SMatrix>(A, UNFACT, b, x0, Eye, UNFACT, MaxIt, RTol);

  double NormB = mPCG.GetNormRHS();

  DVector x;
  mPCG.GetSolution(x);

  INTEGER Iter = -1;
  const double *ResHistory = mPCG.GetResHistory(Iter);
  ...
  const char *out_file1 = "Test_PCG.relres1";
  FILE *fid = fopen(out_file1, "w");
  for (INTEGER i = 0; i < Iter; i++) {
    fprintf(fid, "%g\n", ResHistory[i]/NormB);
  }
  fclose(fid);
  printf("Relative residual history is written to file %s. Use the Matlab program Test_PCG.m to plot it out!\n", out_file1);

  // Test Solve() : No preconditioner, actual residual ----------------------

  mPCG.Solve<SMatrix, SMatrix>(A, UNFACT, b, x0, Eye, UNFACT, MaxIt, RTol, true);

  mPCG.GetSolution(x);

  ResHistory = mPCG.GetResHistory(Iter);

  A.MatVec(x, r, NORMAL);
  r.Subtract(b);
  NormR = r.Norm2();

  printf("Test Solve() : No preconditioner, convergence monitored with true residual\n");
  printf("Number of iterations: %ld\n", (long)Iter);
  printf("Actural relative residual: %g\n", NormR/NormB);
  printf("Relative residual reported by the solver: %g\n", ResHistory[Iter-1]/NormB);

  const char *out_file2 = "Test_PCG.relres2";
  ...
  mPCG.Solve<SMatrix, SMatrix>(A, UNFACT, b, x0, M, UNFACT, MaxIt, RTol);
  ...
  const char *out_file3 = "Test_PCG.relres3";
  ...
  // Test Solve() : With preconditioner, actual residual ----------------------

  mPCG.Solve<SMatrix, SMatrix>(A, UNFACT, b, x0, M, UNFACT, MaxIt, RTol, true);
  ...
  printf("Test Solve() : With preconditioner, convergence monitored with true residual\n");
  printf("Number of iterations: %ld\n", (long)Iter);
  printf("Actural relative residual: %g\n", NormR/NormB);
  printf("Relative residual reported by the solver: %g\n", ResHistory[Iter-1]/NormB);

  const char *out_file4 = "Test_PCG.relres4";
