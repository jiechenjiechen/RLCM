// This program performs kernel ridge regression by using the Fourier
// method. The actual machine learning task can be either regression,
// binary classification, or multiclass classification.
//
// The Fourier method requires a parameter Rank, which is the number
// of Fourier random features. For large data sets, this program may
// not be able to handle the test data all at once. Hence, a parameter
...
// isotropic Gaussian, isotropic Laplace, and product Laplace. For
// their definitions, see the corresponding .hpp files under
// ${CMATRIX_DIR}/src/Kernels/. The required paramters are sigma
// (bandwidth) and lambda (regularization). The global scaling s is
// always 1.
//
// Different from other KRR methods, this method does not support the
...
// Usage:
//
//   KRR_Fourier.ex NumThreads FileTrain FileTest NumClasses d Seed
//   Rank Budget Num_sigma List_sigma Num_lambda List_lambda
//
...
int main(int argc, char **argv) {
  ...
  if (LibSVM_IO::ReadData(FileTrain, Xtrain, ytrain, d) == false) {
    printf("KRR_Fourier: Error reading input data!\n");
    return 0;
  }
  ...
  KRR_Fourier<KernelType, PointFormat, PointArrayFormat> mKRR_Fourier;
  ...
  for (INTEGER j = 0; j < Num_sigma; j++) {
    ...
    for (INTEGER k = 0; k < Num_lambda; k++) {
      ...
      // Training
      START_CLOCK;
      double MemEst = mKRR_Fourier.Train(Xtrain, mKernel, lambda, Rank, Seed);
      END_CLOCK;
      ...
      if (NumClasses <= 2) {
        if (Budget <= 0) {
          mKRR_Fourier.Test(Xtest, ytrain, mKernel, ytest_predict);
        }
        else {
          ytest_predict.Init(Xtest.GetN());
          INTEGER BatchSize = (INTEGER)ceil((double)Budget/Rank);
          ...
          while (RemainSize > 0) {
            ...
            mKRR_Fourier.Test(Xtest_batch, ytrain, mKernel,
                              ytest_predict_batch);
      ...
      printf("KRR_Fourier: r = %ld, param = %g %g, perf = %g, time = %g %g, mem = %g\n", (long)Rank, sigma, lambda, Perf, TimeTrain, TimeTest, MemEst); fflush(stdout);
