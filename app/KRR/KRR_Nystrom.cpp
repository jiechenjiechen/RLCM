// This program performs kernel ridge regression by using the Nystrom
// method. The actual machine learning task can be either regression,
// binary classification, or multiclass classification.
//
// The Nystrom method requires a parameter Rank, which is the rank of
// Nystrom approximation. For large data sets, this program may not be
// able to handle the test data all at once. Hence, a parameter Budget
// is used to split the test data into batches, each handled at a
...
// Usage:
//
//   KRR_Nystrom.ex NumThreads FileTrain FileTest NumClasses d Seed
...
int main(int argc, char **argv) {
  ...
  if (LibSVM_IO::ReadData(FileTrain, Xtrain, ytrain, d) == false) {
    printf("KRR_Nystrom: Error reading input data!\n");
    return 0;
  }
  ...
  KRR_Nystrom<KernelType, PointFormat, PointArrayFormat> mKRR_Nystrom;
  ...
  for (INTEGER j = 0; j < Num_sigma; j++) {
    ...
    for (INTEGER k = 0; k < Num_lambda; k++) {
      ...
      // Training
      START_CLOCK;
      double MemEst = mKRR_Nystrom.Train(Xtrain, mKernel, lambda, Rank, Seed);
      END_CLOCK;
      ...
      if (NumClasses <= 2) {
        if (Budget <= 0) {
          mKRR_Nystrom.Test(Xtest, ytrain, mKernel, ytest_predict);
        }
        else {
          ...
          while (RemainSize > 0) {
            ...
            mKRR_Nystrom.Test(Xtest_batch, ytrain, mKernel,
                              ytest_predict_batch);
