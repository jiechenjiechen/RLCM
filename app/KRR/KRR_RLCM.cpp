...
// Usage:
//
//   KRR_RLCM.ex NumThreads FileTrain FileTest NumClasses d Seed Rank
//   Budget Num_sigma List_sigma Num_lambda List_lambda Par
//   DiagCorrect Refinement
//
...
int main(int argc, char **argv) {
  ...
  if (strcmp(ParString, "RAND") == 0) {
    ...
    printf("KRR_RLCM. Error: Unkown partitioning method!\n");
    return 0;
  }
  ...
  if (LibSVM_IO::ReadData(FileTrain, Xtrain, ytrain, d) == false) {
    printf("KRR_RLCM: Error reading input data!\n");
    return 0;
  }
  ...
  printf("KRR_RLCM: pretrain time = %g\n", TimePreTrain); fflush(stdout);
  ...
  for (INTEGER j = 0; j < Num_sigma; j++) {
    ...
    for (INTEGER k = 0; k < Num_lambda; k++) {
      ...
      if (NumClasses <= 2) {
        if (Budget <= 0) {
          mKRR_RLCM.Test(Xtrain, Xtest, ytrain, mKernel, ytest_predict);
        }
        else {
          ...
          while (RemainSize > 0) {
            ...
            mKRR_RLCM.Test(Xtrain, Xtest_batch, ytrain, mKernel,
                           ytest_predict_batch);
