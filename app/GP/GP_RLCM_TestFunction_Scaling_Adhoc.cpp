...
// method and the NoName test function. This program is an ad hoc
// modification of GP_RLCM_TestFunction_Scaling.cpp in that the
// prediction does not count in noise.
//
...
int main(int argc, char **argv) {
  ...
  // Remove noise
  DVector dytest;
  dy.GetBlock(IdxTest, ytest.GetN(), dytest);
  ytest.Subtract(dytest);
  ...
  // Remove noise
  ytest_stddev.Square();
  ytest_stddev.Subtract(hat_lambda);
  ytest_stddev.Sqrt();
