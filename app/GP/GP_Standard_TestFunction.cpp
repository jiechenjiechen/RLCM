// This program performs GP analysis (MLE, kriging) by using the
// Standard method and the NoName test function.
//
...
// Usage:
//
//   GP_Standard.ex NumThreads Dim Lower Upper noise Num_sigma1
//   List_sigma1 Num_sigma2 List_sigma2 Num_tau List_tau Seed
...
int main(int argc, char **argv) {
  ...
  if (OutputRandomField) {
    WriteRandomFieldToFile(y, d, Dim, Lower, Upper, 0, NULL,
                           RandomFieldFileBasename, "GP_Standard_TestFunction");
  }
  ...
  MLE_Standard<AnisotropicGaussian, DPoint, DPointArray> mMLE;
  START_CLOCK;
  INTEGER t = 0;
  for (INTEGER i = 0; i < Num_sigma1; i++) {
    ...
    for (INTEGER j = 0; j < Num_sigma2; j++) {
      ...
      for (INTEGER k = 0; k < Num_tau; k++) {
        ...
        printf("MLE_Standard: Grid search sigma1 = %g, sigma2 = %g, tau = %g, loglik = %.16e\n", sigma1, sigma2, tau, mLogLik[t]); fflush(stdout);
        t++;
      }
    }
  }
  ...
  if (IsComputeFisher) {
    ...
    for (INTEGER i = 0; i < ListLength2; i++) {
      ...
      printf("MLE_Standard: Fisher info sigma1 = %g, sigma2 = %g, tau = %g, loglik = %.16e\n", sigma1, sigma2, tau, mLogLik2[i]); fflush(stdout);
    }
    ...
    printf("GP_Standard_TestFunction: Stderr sigma1 = %g, sigma2 = %g, tau = %g\n", mStderr[0], mStderr[1], mStderr[2]); fflush(stdout);
    ...
    if (OutputFisher) {
      WriteFisherToFile(Fisher, FisherFileName, "GP_Standard_TestFunction");
    }

  }

  // Use estimated parameters to do kriging
  Kriging_Standard<AnisotropicGaussian, DPoint, DPointArray> mKriging;
  ...
  printf("GP_Standard_TestFunction: Kriging time = %gs\n", TimeKriging); fflush(stdout);
  ...
  if (OutputPredictions) {
    WritePredictionsToFile(ytest, ytest_predict, ytest_stddev,
                           PredictionsFileName, "GP_Standard_TestFunction");
  }
