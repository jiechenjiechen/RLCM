...
// Usage:
//
//   GP_RLCM_NoNugget.ex NumThreads d Dim Lower Upper alpha ell nu
//   Num_alpha List_alpha Num_ell List_ell Num_nu List_nu r
...
int main(int argc, char **argv) {
  ...
  printf("GP_RLCM_NoNugget: Sampling time = %gs\n", TimeSampling); fflush(stdout);
  ...
  if (OutputRandomField) {
    ...
    WriteRandomFieldToFile(y_grid_order, d, Dim, Lower, Upper, NUM_PARAM, Param,
                           RandomFieldFileBasename, "GP_RLCM_NoNugget");
  ...
  if (IsComputeFisher) {
    ...
    printf("GP_RLCM_NoNugget: Stderr alpha = %g, ell = %g, nu = %g\n", mStderr[0], mStderr[1], mStderr[2]); fflush(stdout);
    ...
    if (OutputFisher) {
      WriteFisherToFile(Fisher, FisherFileName, "GP_RLCM_NoNugget");
    }
  ...
  printf("GP_RLCM_NoNugget: Kriging time = %gs\n", TimeKriging); fflush(stdout);
  ...
  if (OutputKrigedRandomField) {
    ...
    WriteRandomFieldToFile(y_all_grid_order, d, Dim, Lower, Upper,
                           NUM_PARAM, HatParam,
                           KrigedRandomFieldFileBasename, "GP_RLCM_NoNugget");
  ...
  if (OutputPredictions) {
    WritePredictionsToFile(ytest, ytest_predict, ytest_stddev,
                           PredictionsFileName, "GP_RLCM_NoNugget");
  }
