...
// Usage:
//
//   GP_Standard_NoNugget.ex NumThreads d Dim Lower Upper alph ell nu
//   Num_alpha List_alpha Num_ell List_ell Num_nu List_nu Seed
...
int main(int argc, char **argv) {
  ...
  printf("GP_Standard_NoNugget: Truth     alpha = %g, ell = %g, nu = %g\n", alpha, ell, nu); fflush(stdout);
  printf("GP_Standard_NoNugget: Estimated alpha = %g, ell = %g, nu = %g, max loglik = %.16e, MLE time = %gs\n", hat_alpha, hat_ell, hat_nu, MaxLogLik, TimeMLE); fflush(stdout);

  // Output all loglik's to file
  if (OutputLogLik) {
    WriteLogLikToFile(NUM_PARAM, ListLength, ListParam, mLogLik,
                      LogLikFileName, "GP_Standard_NoNugget");
  }
  ...
  if (IsComputeFisher) {
    ...
    printf("GP_Standard_NoNugget: Stderr alpha = %g, ell = %g, nu = %g\n", mStderr[0], mStderr[1], mStderr[2]); fflush(stdout);
    ...
    if (OutputFisher) {
      WriteFisherToFile(Fisher, FisherFileName, "GP_Standard_NoNugget");
    }
  ...
  printf("GP_Standard_NoNugget: Kriging time = %gs\n", TimeKriging); fflush(stdout);
  ...
  if (OutputPredictions) {
    WritePredictionsToFile(ytest, ytest_predict, ytest_stddev,
                           PredictionsFileName, "GP_Standard_NoNugget");
  }
