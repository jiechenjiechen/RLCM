...
// Usage:
//
//   GP_Standard_TestFunction_Scaling.ex NumThreads Dim Lower Upper
//   noise Num_alpha List_alpha Num_sigma List_sigma Num_tau List_tau
...
int main(int argc, char **argv) {
  ...
  if (OutputRandomField) {
    WriteRandomFieldToFile(y, d, Dim, Lower, Upper, 0, NULL,
                           ...
                           "GP_Standard_TestFunction_Scaling");
  }
  ...
  MLE_Standard<IsotropicGaussian, DPoint, DPointArray> mMLE;
  START_CLOCK;
  INTEGER t = 0;
  for (INTEGER i = 0; i < Num_alpha; i++) {
    ...
    for (INTEGER j = 0; j < Num_sigma; j++) {
      ...
      for (INTEGER k = 0; k < Num_tau; k++) {
        ...
        printf("MLE_Standard: Grid search alpha = %g, sigma = %g, tau = %g, loglik = %.16e\n", alpha, sigma, tau, mLogLik[t]); fflush(stdout);
        t++;
      }
    }
  }
  ...
  printf("GP_Standard_TestFunction_Scaling: Estimated alpha = %g, sigma = %g, tau = %g, max loglik = %.16e, MLE time = %gs\n", hat_alpha, hat_sigma, hat_tau, MaxLogLik, TimeMLE); fflush(stdout);

  // Output all loglik's to file
  if (OutputLogLik) {
    WriteLogLikToFile(NUM_PARAM, ListLength, ListParam, mLogLik,
                      LogLikFileName, "GP_Standard_TestFunction_Scaling");
  }
  ...
  if (IsComputeFisher) {
    ...
    for (INTEGER i = 0; i < ListLength2; i++) {
      ...
      printf("MLE_Standard: Fisher info alpha = %g, sigma = %g, tau = %g, loglik = %.16e\n", alpha, sigma, tau, mLogLik2[i]); fflush(stdout);
    }
    ...
    printf("GP_Standard_TestFunction_Scaling: Stderr alpha = %g, sigma = %g, tau = %g\n", mStderr[0], mStderr[1], mStderr[2]); fflush(stdout);
  ...
  Kriging_Standard<IsotropicGaussian, DPoint, DPointArray> mKriging;
  ...
  printf("GP_Standard_TestFunction_Scaling: Kriging time = %gs\n", TimeKriging); fflush(stdout);
